...
This module is intentionally standalone and does not require changes to
assets.py, engine.py, or app.py. It can be removed at any time without
impacting core simulation behavior.
"""
...
def _build_extension_event(base_time: datetime, profile: Dict[str, str], event_type: str) -> Dict[str, str]:
    if event_type == "zigbee_replay_attack":
        return {
            ...
            "mitigation_recommendation": (
                "Rotate Zigbee network keys, re-pair trusted devices, and disable "
                "permit-join mode after onboarding."
            ),
    ...
    return {
        ...
        "mitigation_recommendation": (
            "Reposition Z-Wave controller, reduce RF interference sources, and verify "
            "secure inclusion mode for all critical nodes."
        ),
        "mac_address": profile["mac_address"],
    }
...
def simulate_with_extensions(scenario: str = "BREACH") -> List[Dict[str, str]]:
    """Run core simulation and append protocol-extension events for attack scenarios."""
    ...
    zigbee_profile = EXTENSION_PROFILES[0]
    zwave_profile = EXTENSION_PROFILES[1]

    for _ in range(random.randint(2, 5)):
        extension_events.append(_build_extension_event(now, zigbee_profile, "zigbee_replay_attack"))
